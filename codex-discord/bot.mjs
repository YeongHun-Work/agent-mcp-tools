/**
 * Discord + Codex CLI Bot
 * - 슬래시 명령어: /ask, /topic, /history
 * - 멘션/DM 메시지도 /ask와 동일하게 처리
 * - 채널별 대화 세션을 SQLite에 저장
 * - Codex CLI(codex)를 spawn으로 호출 (shell:false)
 */
...
// 상수 정의
// ─────────────────────────────────────────────
const CODEX_TIMEOUT_MS   = 180_000;  // 3분
...
/**
 * Codex CLI를 spawn으로 실행하고 stdout을 반환한다.
 * @param {string} prompt  - Codex에 전달할 프롬프트
...
function callCodex(prompt) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      proc.kill('SIGTERM');
      reject(new Error(`Codex CLI 타임아웃 (${CODEX_TIMEOUT_MS / 1000}초 초과)`));
    }, CODEX_TIMEOUT_MS);
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        reject(new Error(`Codex CLI 종료 코드 ${code}: ${stderr.trim()}`));
      }
    });
...
/**
 * 채널 세션 컨텍스트와 topic을 이용해 Codex 프롬프트를 구성하고 호출한다.
 * @param {string} channelId
 * @param {string} question
 * @returns {Promise<string>} - Codex 응답
 */
async function askCodex(channelId, question) {
  const session      = getSession(channelId);
  ...
  const answer = await callCodex(prompt);
...
const commands = [
  new SlashCommandBuilder()
    .setName('ask')
    .setDescription('Codex에게 질문합니다 (채널 세션 컨텍스트 유지)')
    .addStringOption((opt) =>
      opt
        .setName('question')
  ...
  new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName('show').setDescription('현재 채널의 대화 히스토리를 출력합니다')
    )
    .addSubcommand((sub) =>
      sub.setName('clear').setDescription('현재 채널의 대화 히스토리를 초기화합니다')
    ),
  new SlashCommandBuilder()
    .setName('skill')
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === 'ask') {
    ...
    // Codex 응답이 느릴 수 있으므로 defer 처리
    await interaction.deferReply();

    try {
      const answer = await askCodex(channelId, question);
...
client.once('ready', async () => {
  ...
  // 봇 활동 상태 설정
  client.user.setActivity('Codex CLI');
